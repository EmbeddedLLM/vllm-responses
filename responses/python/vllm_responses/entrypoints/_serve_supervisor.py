    ...
    try:
        ...
        if spec.mcp_runtime is not None:
            mcp_runtime_cmd = [
                ...
                "--host",
                spec.mcp_runtime.host,
                "--port",
                str(spec.mcp_runtime.port),
                "--log-level",
                "error",
            ]
            print(f"[serve] starting mcp-runtime: {shlex.join(mcp_runtime_cmd)}", file=sys.stderr)
            mcp_runtime_proc = subprocess.Popen(
                mcp_runtime_cmd,
                env=dict(os.environ),
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
            ...
            procs.append(("mcp-runtime", mcp_runtime_proc))
            if mcp_runtime_proc.stdout is not None:
                threading.Thread(
                    target=stream_lines,
                    args=("mcp-runtime| ", mcp_runtime_proc.stdout),
                    daemon=True,
                ).start()
            ...
            try:
                wait_http_ready(
                    name="mcp-runtime",
                    url=spec.mcp_runtime.ready_url,
                    timeout_s=60.0,
                    interval_s=2.0,
                    abort_proc=mcp_runtime_proc,
                )
            except Exception:
                code = mcp_runtime_proc.poll()
                if code is not None:
                    print(
                        f"[serve] mcp-runtime exited during startup (code={code}). shutting down.",
                        file=sys.stderr,
                    )
                    return code or 1
                raise
