...
def _code_interpreter_dir_from_spec() -> Path:
    ...
    if spec is None or not spec.submodule_search_locations:
        raise ServeSpecError(
            "[serve] error: failed to locate `vllm_responses.tools.code_interpreter` package data. "
...
def _build_code_interpreter_spawn_spec(
    *,
    ...
    if ci_bin.exists():
        ...
        dev_bun_fallback = env.get_bool("VR_CODE_INTERPRETER_DEV_BUN_FALLBACK", False)
        if dev_bun_fallback and (code_interpreter_dir / "src/index.ts").exists():
            bun_bin = shutil.which("bun")
            if not bun_bin:
                raise ServeSpecError(
                    "[serve] error: VR_CODE_INTERPRETER_DEV_BUN_FALLBACK=1 but `bun` was not found on PATH.",
                    exit_code=2,
                )
            cmd = [
                bun_bin,
            ...
            raise ServeSpecError(
                ...
                "  - For source checkouts, you can set VR_CODE_INTERPRETER_DEV_BUN_FALLBACK=1 and install Bun.\n"
...
def _build_mcp_runtime_spec(*, env: EnvLookup) -> McpRuntimeSpec:
    runtime_url_raw, runtime_url_set = env.get("VR_MCP_BUILTIN_RUNTIME_URL")
    runtime_url = (
        runtime_url_raw.strip()
        if runtime_url_set and runtime_url_raw is not None and runtime_url_raw.strip()
        else "http://127.0.0.1:5981"
    )
    ...
    try:
        ...
        raise ServeSpecError(
            f"[serve] error: invalid VR_MCP_BUILTIN_RUNTIME_URL={runtime_url!r}: {exc}",
            exit_code=2,
        ) from exc

    if parsed.scheme.lower() != "http":
        raise ServeSpecError(
            "[serve] error: VR_MCP_BUILTIN_RUNTIME_URL must use `http://`.",
            exit_code=2,
        )
    ...
    if host not in {"127.0.0.1", "localhost"}:
        raise ServeSpecError(
            "[serve] error: VR_MCP_BUILTIN_RUNTIME_URL must use loopback host "
            "(`127.0.0.1` or `localhost`) in `serve` mode.",
            exit_code=2,
        )
    ...
    if parsed_port is None:
        raise ServeSpecError(
            "[serve] error: VR_MCP_BUILTIN_RUNTIME_URL must include an explicit port.",
            exit_code=2,
        )

    if parsed.path not in {"", "/"} or parsed.query or parsed.fragment:
        raise ServeSpecError(
            "[serve] error: VR_MCP_BUILTIN_RUNTIME_URL must not include path, query, or fragment.",
            exit_code=2,
        )
...
def build_serve_spec(
    ...
    *,
    ...
    upstream_env, upstream_env_set = env.get("VR_LLM_API_BASE")
    ...
    gateway_host = (
        gateway_host_arg if gateway_host_arg is not None else env.get_str("VR_HOST", "0.0.0.0")
    )
    gateway_port_arg = getattr(args, "gateway_port", None)
    gateway_port = (
        gateway_port_arg if gateway_port_arg is not None else env.get_int("VR_PORT", 5969)
    )
    gateway_workers_arg = getattr(args, "gateway_workers", None)
    gateway_workers = (
        gateway_workers_arg if gateway_workers_arg is not None else env.get_int("VR_WORKERS", 1)
    )
    metrics_enabled = env.get_bool("VR_METRICS_ENABLED", True)
    mcp_config_path, mcp_config_path_set = env.get("VR_MCP_CONFIG_PATH")
    mcp_runtime: McpRuntimeSpec | None = None
    if mcp_config_path_set and mcp_config_path and mcp_config_path.strip():
        mcp_runtime = _build_mcp_runtime_spec(env=env)

    ci_mode_arg = getattr(args, "code_interpreter", None)
    ci_mode = (
        ci_mode_arg
        if ci_mode_arg is not None
        else env.get_str("VR_CODE_INTERPRETER_MODE", "spawn")
    )
    ...
    ci_port = (
        ci_port_arg if ci_port_arg is not None else env.get_int("VR_CODE_INTERPRETER_PORT", 5970)
    )
    ci_workers_arg = getattr(args, "code_interpreter_workers", None)
    ci_workers = (
        ci_workers_arg
        if ci_workers_arg is not None
        else env.get_int("VR_CODE_INTERPRETER_WORKERS", 0)
    )
    ...
    openai_key, openai_key_set = env.get("VR_OPENAI_API_KEY")
    ...
    if upstream_flag is not None:
        ...
        if upstream_env_set and upstream_env and upstream_env != upstream_base_url:
            notices.append(
                f"[serve] notice: ignoring VR_LLM_API_BASE={upstream_env!r} in favor of --upstream."
            )
        ...
        raise ServeSpecError(
            ...
            "  - `VR_LLM_API_BASE=...` (external upstream via env/.env)\n"
