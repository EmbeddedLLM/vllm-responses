...
WEB_SEARCH_BUILTIN_MCP_SERVERS: dict[str, BuiltinMcpServerDefinition] = {
    _EXA_SERVER_LABEL: BuiltinMcpServerDefinition(
        ...
        # Managed by the gateway, remote by transport. Exa supports
        # operator-supplied API keys in the MCP URL, so this entry is
        # materialized at runtime instead of being fully static.
        build_server_entry=_build_exa_server_entry,
    ),
    _FETCH_SERVER_LABEL: BuiltinMcpServerDefinition(
        server_label=_FETCH_SERVER_LABEL,
        # Managed by the gateway, local by transport. This stdio MCP server
        # backs page fetch/open_page support.
        server_entry={
            "command": "uvx",
