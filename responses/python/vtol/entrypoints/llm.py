...
from vtol.configs.mock_llm import MockLLMConfig
from vtol.entrypoints.state import VtolAppState
from vtol.utils import uuid7_str
from vtol.utils.cassette_replay import CassetteReplayer, CassetteReplayError, stream_sse_chunks


def _get_cassette_replayer(app: FastAPI) -> CassetteReplayer | None:
    return app.state.vtol.cassette_replayer
...
async def lifespan(app: FastAPI):
    ...
    if mock_cfg.mode == "replay":
        ...
        if not scenarios_json:
            raise RuntimeError(
                "VTOL_MOCK_LLM_MODE=replay requires VTOL_MOCK_LLM_SCENARIOS to be set."
            )
        app.state.vtol.cassette_replayer = CassetteReplayer.from_env(
    ...
    if replayer is not None:
        try:
            cassette_resp = replayer.next_response(
                scenario=request.headers.get("X-VTOL-Scenario"),
                method="POST",
