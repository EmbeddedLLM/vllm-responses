...
from vtol.utils import mask_string
...
def make_request_log_str(request: Request, status_code: int | None = None) -> str:
    ...
    msg = f'{request.state.vtol.id} - "{request.method} {request.url.path}{query}"'
...
def make_response(
    ...
    *,
    ...
    if status_code == 500:
        ...
        log_fn = logger.info
    elif isinstance(exception, (VtolException, HTTPException)):
        log_fn = logger.info
    else:
        log_fn = logger.warning
    if exception:
...
async def _request_validation_exc_handler(request: Request, exc: RequestValidationError):
    ...
    try:
        ...
        if content is None:
            content = repr(exc)
        logger.exception(f"{request.state.vtol.id} - Failed to parse error data: {content}")
        message = str(exc)
        return ORJSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={
