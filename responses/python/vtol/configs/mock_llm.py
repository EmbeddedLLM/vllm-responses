...
class MockLLMConfig(BaseSettings):
    """Configuration for the mock upstream LLM server (`vtol.entrypoints.llm`).

    Notes:
    - We intentionally keep this separate from `EnvConfig` because:
      - `VTOL_MOCK_LLM_SCENARIOS` can be large (avoid printing/logging it).
      - The gateway runtime does not need mock-LLM settings.
    - Env vars use the `VTOL_MOCK_LLM_` prefix to make scope unambiguous.
    """

    model_config = SettingsConfigDict(
        env_prefix="vtol_mock_llm_",
        env_file=".env",
    ...
    cassette_dir: str = "responses/python/tests/cassettes/chat_completion"
    ...
    def scenarios_mapping(self) -> dict[str, list[str]]:
        ...
        if not isinstance(data, dict):
            raise ValueError("VTOL_MOCK_LLM_SCENARIOS must be a JSON object mapping name->list.")
