...
layer (ResponseStore) owns its own schema and semantics under `vtol.responses_core`.
"""
...
def _create_db_engine(
    *,
    ...
    db_dialect = ENV_CONFIG.db_dialect
    if db_dialect == "sqlite":
        ...
        connect_args = {}
        if "asyncpg" in ENV_CONFIG.db_path:
...
def create_db_engine_async() -> AsyncEngine:
    engine = _create_db_engine(engine_create_fn=create_async_engine)
    return engine
